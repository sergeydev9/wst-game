  ...
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      if (!rows.length) {
        ...
        const resetResponse = await emailService.sendResetCode(
          rows[0].user_email,
          code
        );
        ...
        if (resetResponse[0].statusCode === 202) {
          ...
          if (resetResponse[0].statusCode === 202) {
            ...
            logError(
              'Error while attempting to send password reset email',
              resetResponse
            );
...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (
        e.message ===
        'duplicate key value violates unique constraint "free_credit_signups_email_key"'
      ) {
