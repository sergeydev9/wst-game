...
export const pickName = () => {
    const leftRandom = Math.floor(Math.random() * left.length)
    const rightRandom = Math.floor(Math.random() * right.length)
    return `${capitalizeFirstLetter(left[leftRandom])} ${capitalizeFirstLetter(right[rightRandom])}`
}
