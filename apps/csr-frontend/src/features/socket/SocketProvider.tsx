...
import {
  setFunFacts,
  setMostSimilar,
  setFetchSimilarStatus,
} from '../fun-facts/funFactsSlice';
import { setUpcomingLines } from '../one-liners/oneLinersSlice';
...
export const SocketProvider: React.FC = ({ children }) => {
  ...
  useEffect(() => {
    ...
    const shouldHaveConnection =
      ['inGame', 'lobby', 'gameCreateSuccess', 'choosingName'].includes(
        playerStatus
      ) && playerId;

    // if user should, but doesn't, have a socket connection, create one and register listeners
    if (shouldHaveConnection && !socket) {
      const connection = io(process.env.NX_SOCKET_BASEURL as string, {
        auth: { token },
      ...
      connection.on('connect', () => {
        ...
        connection.emit(types.PLAYER_JOINED_GAME, {
          id: playerId,
      ...
      connection.on('disconnect', (reason) => {
        ...
        if (
          ...
          dispatch(
            showLoaderMessage('Connection to server lost, reconnecting...')
          );
      ...
      const fetchMostSimilar = () => {
        ...
        connection.emit(
          types.FETCH_MOST_SIMILAR,
          {},
          (cb: string | payloads.FetchMostSimilar) => {
      ...
      connection.on(
        types.SET_CURRENT_PLAYERS,
        (message: payloads.SetCurrentPlayers) => {
      ...
      connection.on(
        types.PLAYER_JOINED_GAME,
        (message: payloads.PlayerEvent) => {
      ...
      connection.on(
        types.SET_HAVE_NOT_ANSWERED,
        (message: payloads.SetHaveNotAnswered) => {
      ...
      connection.on(types.GAME_FINISHED, () => {
        dispatch(
          showError('The game you are attempting to join has already finished.')
        );
        clear();
        history.push('/');
        connection.close(); // close  and delete the socket
      ...
      if (
        ['inProgress', 'lobby', 'postGame'].includes(gameStatus) &&
  ...
  return (
    <socketContext.Provider
      value={{ socket, setSocket, sendMessage, shouldBlock, setShouldBlock }}
    >
      {children}
    </socketContext.Provider>
  );
